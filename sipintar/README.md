...
Proyek ini memiliki kode, konfigurasi, database, akun, role, dan **satu superadmin sendiri**. Seluruh kode aplikasi berada dalam folder `sipintar`. Folder ini bisa dipasang sendiri di root domain atau subdirektori dengan nama apa pun.
...
|---|---|
| pegawai | Lihat barang dan buat transaksi keluar atas nama sendiri |
| petugas | Kelola barang, transaksi, dan laporan |
| pemantau | Lihat barang dan laporan |
...
Cookie sesi proyek ini bernama `SIPINTAR_INVENTORY_SESSION`, dibatasi ke path pemasangan. Login/logout proyek lain tidak mengubah sesi di sini. Password disimpan sebagai hash; POST memerlukan CSRF. Percobaan login dibatasi. Tabel `users` dan password lama tidak digunakan untuk autentikasi baru.
...
3. Salin `config.example.php` ke lokasi di luar document root, misalnya `/etc/sipintar/config.php`, lalu isi koneksi database proyek ini dan konfigurasi Portal Data. Set `SIPINTAR_CONFIG` pada PHP-FPM/Apache serta CLI/cron. File harus dapat dibaca pengguna PHP. `cookie_secure=false` hanya untuk localhost HTTP.
4. Dari folder proyek ini, jalankan:

```bash
export SIPINTAR_CONFIG=/etc/sipintar/config.php
php bin/console.php migrate
php bin/console.php migrate-business
read -rsp 'Password superadmin (min. 12 karakter): ' SIPINTAR_ADMIN_PASSWORD
export SIPINTAR_ADMIN_PASSWORD
php bin/console.php superadmin superadmin
unset SIPINTAR_ADMIN_PASSWORD
```
...
- Riwayat transaksi dibatasi 100 baris per halaman (maksimal 200 melalui API), dengan detail diambil secara batch.
- Filter, ringkasan transaksi, PDF, dan Excel mengikuti **halaman riwayat yang dimuat**. UI menampilkan batasan ini. Laporan lintas seluruh riwayat memerlukan pengembangan endpoint laporan tersendiri.
- Katalog barang masih dimuat lengkap untuk pilihan formulir dan ringkasan stok.
- Stok memakai transaksi InnoDB dan penguncian baris. Jumlah negatif/stok kurang ditolak. Edit/hapus transaksi mengoreksi stok secara atomik; pengambilan bersamaan diuji.
- Pegawai hanya membuat transaksi keluar atas nama sendiri. Petugas dapat memilih pegawai lain dari sumber Portal. Barang dengan riwayat tidak bisa diganti namanya/dihapus; nama duplikat pada database lama perlu dibereskan sebelum ditransaksikan.
