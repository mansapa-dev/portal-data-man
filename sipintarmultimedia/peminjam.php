...
if (!sip_user()) { header('Location: index.php'); exit; }
$currentUser = sip_require('multimedia', 'borrowings.create');
$conn = sip_db('multimedia');
$success_code = ''; $error_msg = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $success_code = (new \Sipintar\Borrowings(new \Sipintar\Database($conn)))->save($_POST, $employee);
    } catch (RuntimeException $e) { if ($e instanceof mysqli_sql_exception) throw $e; $error_msg = $e->getMessage(); }
}
?>
...
<form method="POST" class="text-center p-4"><input type="hidden" name="_csrf" value="<?= sip_e(sip_csrf()) ?>"><button name="form_type" value="logout">Keluar</button></form></body>
</html>
