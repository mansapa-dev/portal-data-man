...
@Injectable() export class ResponseInterceptor implements NestInterceptor {
 intercept(_:ExecutionContext,next:CallHandler):Observable<unknown>{return next.handle().pipe(map((value:any)=>{
   ...
   const hasData=value!==null&&typeof value==='object'&&Object.prototype.hasOwnProperty.call(value,'data');
   return {success:true,message:value?.message??'Permintaan berhasil.',data:hasData?value.data:value??null};
 }));}
}
