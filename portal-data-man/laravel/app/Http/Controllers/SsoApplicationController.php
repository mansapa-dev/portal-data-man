...
class SsoApplicationController extends Controller
{
    ...
    private function validated(Request $request, bool $partial = false, ?ApplicationClient $client = null): array
    {
        ...
        return $request->validate(['name' => [$required, 'string', 'min:2', 'max:150'], 'slug' => [$required, 'regex:/^[a-z0-9-]+$/', 'max:100', Rule::unique('ApplicationClient', 'slug')->ignore($client?->id)], 'applicationType' => [$partial ? 'sometimes' : 'required', Rule::in(['SSO', 'SERVICE'])], 'description' => ['sometimes', 'nullable', 'string'], 'launchUrl' => [$service ? 'sometimes' : 'nullable', 'url:http,https', 'max:2000'], 'redirectUris' => [$service ? 'sometimes' : $required, 'array', $service ? 'max:0' : 'min:1'], 'redirectUris.*' => ['url', 'max:2000'], 'postLogoutRedirectUris' => ['sometimes', 'array'], 'postLogoutRedirectUris.*' => ['url', 'max:2000'], 'status' => ['sometimes', Rule::in(['ACTIVE', 'INACTIVE'])]]);
    }
