...
set_exception_handler(function(Throwable$e):never{$dir=__DIR__.'/storage/logs';if(!is_dir($dir))mkdir($dir,0775,true);error_log(sprintf("[%s] %s\n%s\n",date(DATE_ATOM),$e->getMessage(),$e->getTraceAsString()),3,$dir.'/app.log');$status=$e instanceof Cbt\Exceptions\DomainException?$e->status:500;$debug=Config::bool('APP_DEBUG');$isApi=str_contains($_SERVER['REQUEST_URI']??'','/api/');$message=$isApi||($e instanceof Cbt\Exceptions\DomainException)||$status<500||$debug?$e->getMessage():'Terjadi kesalahan pada sistem.';Cbt\Core\Response::error($message,$status)->send();});
