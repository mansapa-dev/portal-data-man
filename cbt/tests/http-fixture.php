...
$questionCount=max(1,min(100,(int)(getenv('CBT_TEST_QUESTIONS')?:40)));
for($i=1;$i<=$questionCount;$i++)$question->execute(['question-'.$i]);
$insert=$pdo->prepare("INSERT INTO students(portal_student_id,nisn,name_snapshot,grade_snapshot,pin_hash) VALUES(?,?,?,'X',?)");
$hash=password_hash('123456',PASSWORD_DEFAULT);
$studentCount=max(1,min(2000,(int)(getenv('CBT_TEST_STUDENTS')?:1300)));
for($i=1;$i<=$studentCount;$i++)$insert->execute(['student-'.$i,str_pad((string)$i,10,'0',STR_PAD_LEFT),'Test Student '.$i,$hash]);
