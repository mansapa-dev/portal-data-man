...
if($request->path==='/'||$request->path==='/index.php'){
 ...
 $html=str_replace('</body>','<script src="assets/js/native-api-adapter.js?v=20260922-image-import-2"></script></body>',$html);
...
$loginNetworkRate=new RateLimitMiddleware($pdo,'login-network',max(100,(int)Config::get('LOGIN_IP_LIMIT',2000)),300);
$router=new Router();
...
$router->post('/api/auth/student/login',[$auth,'studentLogin'],[$csrf,$loginNetworkRate,$studentLoginRate,$audit('STUDENT_LOGIN','Student')]);
$router->post('/api/auth/staff/login',[$auth,'staffLogin'],[$csrf,$loginNetworkRate,$staffLoginRate,$audit('STAFF_LOGIN','User')]);
...
$router->post('/api/student/exams/{id}/heartbeat',[$student,'heartbeat'],[$studentAuth,$csrf]);
...
$router->post('/api/student/support-tickets/request',[$supportTickets,'create'],[$supportTicketRate,$csrf,$audit('SUPPORT_TICKET_CREATED','SupportTicket')]);
$router->get('/api/student/support-tickets',[$supportTickets,'studentIndex'],[$studentSupportAuth]);
