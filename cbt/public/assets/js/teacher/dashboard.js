(async function () {
  ...
  function render(section) {
    ...
    if (section === 'results') {
      ...
      const grade = el('select'), studentClass = el('select'), subject = el('select'), print = el('button', undefined, 'teacher-print-button'), excel = el('button', undefined, 'teacher-excel-button');
      ...
      const examClasses = data.ujianList.flatMap(x => String(x.nama_kelas_target || '').split(',').map(v => v.trim()).filter(Boolean));
      const grades = [...new Set([...data.ujianList.map(x=>String(x.tingkat||'').trim()),...data.hasilList.map(x => String(x.tingkat || '').trim())].filter(Boolean))].sort();
      const classes = [...new Set([...examClasses,...data.hasilList.map(x => String(x.kelas || '').trim())].filter(Boolean))].sort((a,b)=>a.localeCompare(b, 'id', {numeric:true}));
      const subjects = [...new Set([...data.ujianList.map(x=>String(x.nama_mapel||'').trim()),...data.hasilList.map(x=>String(x.nama_mapel||'').trim())].filter(Boolean))].sort((a,b)=>a.localeCompare(b,'id'));
      ...
      const gradeField = el('label', undefined, 'teacher-filter-field'), classField = el('label', undefined, 'teacher-filter-field'), subjectField = el('label', undefined, 'teacher-filter-field');
      gradeField.append(el('span', 'Tingkatan'), grade);
      classField.append(el('span', 'Kelas'), studentClass);
      subjectField.append(el('span', 'Mata Pelajaran'), subject);
      const resultTable = el('div', undefined, 'teacher-result-table');
      let filteredResults = [];
      const updateResults = () => {
        filteredResults = data.hasilList.filter(x => (grade.value === 'ALL' || String(x.tingkat) === grade.value) && (studentClass.value === 'ALL' || String(x.kelas) === studentClass.value) && (subject.value === 'ALL' || String(x.nama_mapel) === subject.value));
        resultTable.replaceChildren(table(
          ['NISN', 'Nama Siswa', 'Kelas', 'Tingkat', 'Mata Pelajaran', 'Ujian', 'Nilai', 'Status'],
      ...
      excel.addEventListener('click', () => {
        ...
        const rows = filteredResults.map(x => ({NISN:x.nomor_ujian,'Nama Siswa':x.nama_siswa,Kelas:x.kelas,Tingkat:x.tingkat||'-','Mata Pelajaran':x.nama_mapel||'Mata Pelajaran Umum',Ujian:x.nama_ujian,Nilai:x.nilai??0,Status:String(x.status||'selesai').toUpperCase()}));
        const sheet = XLSX.utils.json_to_sheet(rows, {header:['NISN','Nama Siswa','Kelas','Tingkat','Mata Pelajaran','Ujian','Nilai','Status']});
        sheet['!cols'] = [{wch:18},{wch:30},{wch:18},{wch:12},{wch:28},{wch:35},{wch:12},{wch:16}];
      ...
      controls.append(gradeField, classField, subjectField, excel, print);
