    ...
    .withSuccessHandler(res => {
      ...
      cont.innerHTML = res.ujianList.map(u => {
        ...
        let tbHtml = hasil.length===0 ? `<tr><td colspan="5" align="center" style="color:var(--text-muted);">Belum ada peserta submit.</td></tr>` :
          hasil.map(h => `<tr><td>${h.nomor_ujian || '-'}</td><td><b>${h.nama_siswa || '-'}</b></td><td>${h.kelas || '-'}</td><td><b style="color:var(--primary-dark); font-size:14px;">${h.nilai !== undefined ? h.nilai : 0}</b></td><td><span class="badge bg-green">${String(h.status || 'selesai').toUpperCase()}</span></td></tr>`).join('');
        ...
        return `
          <div class="card" style="border:1px solid var(--border); padding:0; overflow:hidden; margin-bottom:20px;">
            <div style="background:var(--primary-soft); padding:16px 20px; border-bottom:1px solid var(--primary-soft-border); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
              <div>
                <h3 style="font-size:16px; font-weight:800; color:var(--primary-dark); margin-bottom:4px;"><i class="fa-solid fa-book-open"></i> ${u.nama_ujian}</h3>
                <p style="font-size:12px; color:var(--text-muted);">Tingkat: ${u.tingkat} | Sesi: ${u.sesi || 1} | Thn Ajaran: ${u.tahun_ajaran||'-'} (${u.semester||'-'}) | Submit: <b>${hasil.length} Siswa</b></p>
              </div>
