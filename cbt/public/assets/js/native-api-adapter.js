(function () {
  ...
  const calls = {
    ...
    async resetSupportTicket(id,reason) { const r=await api(`api/staff/support-tickets/${id}/reset`,'POST',{reason});return {success:true,...r.data,message:r.message}; },
    ...
    async catatPelanggaranServer(studentId, examId, type='TAB_HIDDEN') { const r=await api(`api/student/exams/${examId}/violations`,'POST',{event_key:`${type.toLowerCase()}:${Date.now()}:${Math.random().toString(36).slice(2)}`,type,client_occurred_at:new Date().toISOString().slice(0,23).replace('T',' ')}); if(r.data.terminated) await closeQueue(); return {success:true,jumlah:r.data.jumlah,dihentikan:r.data.terminated,hasil:r.data.hasil}; },
