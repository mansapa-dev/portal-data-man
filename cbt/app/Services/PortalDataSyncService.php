...
final class PortalDataSyncService
{
 ...
 {
  $type=strtoupper($type);if(!in_array($type,['STUDENTS','TEACHERS','EMPLOYEES','CLASSES','ACADEMIC_YEARS','SEMESTERS'],true))throw new \InvalidArgumentException('Jenis sinkronisasi tidak valid.');
 ...
 private function upsert(string$type,array$d):string{return match($type){'STUDENTS'=>$this->student($d),'TEACHERS'=>$this->teacher($d),'EMPLOYEES'=>$this->employee($d),'CLASSES'=>$this->schoolClass($d),'ACADEMIC_YEARS'=>$this->academicYear($d),'SEMESTERS'=>$this->semester($d)};}
 ...
 private function employee(array$d):string{
  $portal=(string)($d['id']??$d['employee_id']??'');$name=trim((string)($d['name']??$d['nama']??''));if($portal===''||$name==='')throw new \UnexpectedValueException('Data pegawai invalid.');
  $existing=$this->row('employees','portal_employee_id',$portal);$v=['portal'=>$portal,'nip'=>$d['nip']??null,'name'=>$name,'position'=>$d['position']??null,'status'=>(filter_var($d['is_active']??(strtoupper((string)($d['status']??'ACTIVE'))==='ACTIVE'),FILTER_VALIDATE_BOOL)?'ACTIVE':'INACTIVE')];
  if($existing&&$this->same($existing,['nip'=>$v['nip'],'name_snapshot'=>$v['name'],'position_snapshot'=>$v['position'],'status'=>$v['status']])){$this->touch('employees',(int)$existing['id']);return'unchanged';}
  if(!$existing){$sql='INSERT INTO employees(portal_employee_id,nip,name_snapshot,position_snapshot,status,last_synced_at) VALUES(:portal,:nip,:name,:position,:status,UTC_TIMESTAMP(3))';$this->db->pdo()->prepare($sql)->execute($v);return'inserted';}
  $sql='UPDATE employees SET nip=:nip,name_snapshot=:name,position_snapshot=:position,status=:status,last_synced_at=UTC_TIMESTAMP(3) WHERE portal_employee_id=:portal';$this->db->pdo()->prepare($sql)->execute($v);return'updated';
 }
 ...
 {
  [$table,$statusColumn,$inactiveValue]=match($type){'STUDENTS'=>['students','is_active',0],'TEACHERS'=>['teachers','status','INACTIVE'],'EMPLOYEES'=>['employees','status','INACTIVE'],'CLASSES'=>['portal_classes','status','INACTIVE'],'ACADEMIC_YEARS'=>['portal_academic_years','is_active',0],'SEMESTERS'=>['portal_semesters','is_active',0]};
