...
final class SupportTicketService
{
    ...
    public function update(string $publicId,array $auth,string $status,string $note):array
    {
        ...
        if(in_array(($auth['role']??''),['TEACHER','EMPLOYEE'],true)&&in_array($status,['OPEN','CLOSED'],true))throw new DomainException('Petugas dapat mengambil atau menyelesaikan tiket.',403);
