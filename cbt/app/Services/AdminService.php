...
final class AdminService
{
 ...
 public function saveUser(array$d):void{if(!preg_match('/^[A-Za-z0-9._-]{4,100}$/',(string)($d['username']??'')))throw new DomainException('Username/NIP tidak valid.',422);if(empty($d['id'])&&strlen((string)($d['password']??''))<12)throw new DomainException('Password akun baru minimal 12 karakter.',422);if(!empty($d['password'])&&strlen((string)$d['password'])<12)throw new DomainException('Password minimal 12 karakter.',422);$role=strtoupper((string)($d['role']??''));if(!in_array($role,['ADMIN','TEACHER'],true))throw new DomainException('Role tidak valid.',422);$d['role']=$role;$d['status_aktif']=filter_var($d['status_aktif']??false,FILTER_VALIDATE_BOOL);try{$this->repo->saveUser($d);}catch(\UnexpectedValueException$e){throw new DomainException($e->getMessage(),422);}}
 ...
 public function teacherDashboard(int$userId,string$role):array{$ids=$this->repo->teacherExamIds($userId,$role==='ADMIN');$all=$this->repo->exams();$list=array_values(array_filter($all,fn($e)=>in_array($e['id'],$ids,true)));return['ujianList'=>$list,'hasilList'=>$this->repo->results($ids),'pelanggaranList'=>$this->repo->violations($ids)];}
 public function importQuestions(array$rows):array{$valid=[];$errors=[];foreach($rows as$i=>$row){try{$exam=(int)($row['ujian_id']??0);if(!$exam&&!empty($row['nama_ujian']))$exam=$this->repo->examIdByName((string)$row['nama_ujian'])??0;$row['ujian_id']=$exam;foreach(['ujian_id','pertanyaan','opsi_a','opsi_b','opsi_c','opsi_d','jawaban_benar']as$key)if(trim((string)($row[$key]??''))==='')throw new \InvalidArgumentException("Kolom {$key} kosong");$row['jawaban_benar']=strtoupper((string)$row['jawaban_benar']);if(!in_array($row['jawaban_benar'],['A','B','C','D','E'],true))throw new \InvalidArgumentException('Jawaban benar harus A-E');$row['poin']=(float)($row['poin']??1);$row['opsi_e']=$row['opsi_e']??'';$valid[]=$row;}catch(\Throwable$e){$errors[]=['row'=>$i+2,'reason'=>$e->getMessage()];}}if(!$valid)throw new DomainException('Tidak ada soal valid untuk diimport.',422);$this->db->transaction(function()use($valid){foreach($valid as$row)$this->repo->saveQuestion($row);});return['total'=>count($rows),'inserted'=>count($valid),'failed'=>count($errors),'errors'=>$errors];}
 public function importUsers(array$rows):array{$valid=[];$errors=[];foreach($rows as$i=>$row){try{$data=['username'=>trim((string)($row['username']??'')),'nama_lengkap'=>trim((string)($row['nama_lengkap']??'')),'password'=>(string)($row['password']??''),'role'=>strtoupper((string)($row['role']??'TEACHER')),'status_aktif'=>true];if(!preg_match('/^[A-Za-z0-9._-]{4,100}$/',$data['username'])||strlen($data['password'])<12||!in_array($data['role'],['ADMIN','TEACHER'],true))throw new \InvalidArgumentException('Username/role/password minimal 12 karakter tidak valid');$valid[]=$data;}catch(\Throwable$e){$errors[]=['row'=>$i+2,'reason'=>$e->getMessage()];}}if(!$valid)throw new DomainException('Tidak ada akun valid untuk diimport.',422);$this->db->transaction(function()use($valid){foreach($valid as$row)$this->repo->saveUser($row);});return['total'=>count($rows),'inserted'=>count($valid),'failed'=>count($errors),'errors'=>$errors];}
}
