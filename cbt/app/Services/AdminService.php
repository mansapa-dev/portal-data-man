...
final class AdminService
{
 ...
 public function teacherLiveSessions(int$userId):array{$sessions=$this->repo->liveSessions($this->repo->teacherExamIds($userId,false));return['sessions'=>$sessions,'summary'=>['active'=>count(array_filter($sessions,fn(array$s):bool=>$s['status']==='IN_PROGRESS')),'answered'=>array_sum(array_column($sessions,'answeredQuestions')),'violations'=>array_sum(array_column($sessions,'violationCount')),'total'=>count($sessions)],'serverTime'=>gmdate(DATE_ATOM),'refreshSeconds'=>10];}
