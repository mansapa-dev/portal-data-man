...
final class AdminStudentService
{
 ...
 public function generatePinsBatch(?string$grade=null,?string$class=null):array{$students=$this->students->all();$updated=0;$this->db->transaction(function()use($students,$grade,$class,&$updated){foreach($students as$s){if(!empty($grade)&&$grade!=='ALL'&&strtoupper((string)($s['grade_snapshot']??''))!==strtoupper($grade))continue;if(!empty($class)&&$class!=='ALL'&&strtoupper((string)($s['class_snapshot']??''))!==strtoupper($class))continue;$pin=(string)random_int(1000,9999);$hash=password_hash($pin,defined('PASSWORD_ARGON2ID')?PASSWORD_ARGON2ID:PASSWORD_DEFAULT);$encrypted=$this->cipher->encrypt($pin);$this->students->setPin((int)$s['id'],$hash,$encrypted);$updated++;}});return['updated'=>$updated];}
