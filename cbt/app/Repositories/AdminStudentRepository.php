...
final class AdminStudentRepository
{
 ...
 public function all():array
 {
  $sql="SELECT listed.*,CASE
        ...
        END ujian_status
        FROM (
          SELECT s.id,s.nisn,s.name_snapshot,s.class_snapshot,s.grade_snapshot,s.academic_year_snapshot,s.is_active,s.last_synced_at,
 ...
 public function setPin(int$id,string$hash):void{$s=$this->db->prepare('UPDATE students SET pin_hash=:hash WHERE id=:id');$s->execute(compact('hash','id'));}
