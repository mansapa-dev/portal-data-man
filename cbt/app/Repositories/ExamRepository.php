...
final class ExamRepository
{
    ...
    public function eligibleForStudent(array $student): array
    {
        $sql = "SELECT e.* FROM exams e WHERE e.status='ACTIVE' AND e.grade=:grade
                ...
                AND (EXISTS(SELECT 1 FROM exam_target_students ts WHERE ts.exam_id=e.id AND ts.student_id=:student_id)
        ...
        $statement->execute(['grade' => $student['grade_snapshot'], 'class_id' => $student['portal_class_id'], 'student_id' => $student['id']]);
    ...
    public function findEligible(int $examId, array $student, bool $lock = false): ?array
    {
        ...
        $statement = $this->db->prepare($sql);
        $statement->execute(['id' => $examId, 'grade' => $student['grade_snapshot'], 'class_id' => $student['portal_class_id'], 'student_id' => $student['id']]);
