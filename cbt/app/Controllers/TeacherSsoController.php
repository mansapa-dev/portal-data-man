...
final class TeacherSsoController
{
    ...
    {
        $session=$_SESSION['oidc_teacher']??[];
        if(!isset($session['state'])||!hash_equals((string)$session['state'],(string)($request->query['state']??'')))return Response::error('State SSO tidak valid.',400);
        $oidc=$this->oidc();$body=$this->post($oidc['token_endpoint'],['grant_type'=>'authorization_code','client_id'=>$this->required('PORTAL_DATA_OIDC_CLIENT_ID'),'code'=>$request->query['code']??'','redirect_uri'=>$this->required('PORTAL_DATA_OIDC_REDIRECT_URI'),'code_verifier'=>$session['verifier']]);
        $claims=$this->verify((string)($body['id_token']??''),(string)$session['nonce'],$oidc);if(!$claims)return Response::error('ID token Portal Data tidak valid.',401);
        $info=$this->userinfo($oidc['userinfo_endpoint'],(string)($body['access_token']??''));$nip=(string)($info['preferred_username']??$claims['preferred_username']??'');
        $employeePortal=(string)($info['portal_employee_id']??$claims['portal_employee_id']??'');$teacherPortal=(string)($info['portal_teacher_id']??$claims['portal_teacher_id']??'');
        $employee=null;$teacher=null;$role='TEACHER';
        if($employeePortal!==''){$q=$this->db->prepare("SELECT * FROM employees WHERE status='ACTIVE' AND (portal_employee_id=:portal OR (nip=:nip AND nip<>'')) LIMIT 1");$q->execute(['portal'=>$employeePortal,'nip'=>$nip]);$employee=$q->fetch()?:null;$role='EMPLOYEE';}
        else{$q=$this->db->prepare("SELECT * FROM teachers WHERE status='ACTIVE' AND (portal_teacher_id=:portal OR (nip=:nip AND nip<>'')) LIMIT 1");$q->execute(['portal'=>$teacherPortal,'nip'=>$nip]);$teacher=$q->fetch()?:null;}
        $person=$employee??$teacher;if(!$person)return Response::error('Personel belum tersinkron dari Portal Data.',403);
