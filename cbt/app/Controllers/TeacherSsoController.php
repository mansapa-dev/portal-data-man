...
final class TeacherSsoController
{
    ...
    {
        Session::destroy();
        $issuer=rtrim((string)Config::get('PORTAL_DATA_OIDC_ISSUER',''),'/');
        $fallback=(string)Config::get('PORTAL_DATA_OIDC_POST_LOGOUT_URI','/guru');
        if($issuer===''){header('Location: '.$fallback);exit;}
        $params=$fallback!==''?['post_logout_redirect_uri'=>$fallback]:[];
        header('Location: '.$issuer.'/logout'.($params?'?'.http_build_query($params):''));exit;
    }
